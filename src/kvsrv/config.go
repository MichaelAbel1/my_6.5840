...
func (cfg *config) StartServer() {
	cfg.kvserver = StartKVServer() // 启动一个键值存储服务器

	kvsvc := labrpc.MakeService(cfg.kvserver) // 为这个服务器创建一个RPC服务接口
	srv := labrpc.MakeServer()                // 创建一个RPC服务器实例（srv）
	srv.AddService(kvsvc)                     // 将之前创建的RPC服务接口添加到RPC服务器实例中
	cfg.net.AddServer(0, srv)                 // 将RPC服务器添加到网络层，端口号为0，表示由系统自动选择一个可用端口。
}
...
func make_config(t *testing.T, unreliable bool) *config {
	...
	runtime.GOMAXPROCS(4) // 设置最大并发处理数为4
	cfg := &config{}
	cfg.t = t
