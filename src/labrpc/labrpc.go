...
type ClientEnd struct {
	...
	ch      chan reqMsg   // copy of Network.endCh   通常是 8 字节
	done    chan struct{} // closed when Network is cleaned up  通常是 8 字节
}
...
func (e *ClientEnd) Call(svcMeth string, args interface{}, reply interface{}) bool {
	...
	// send the request.
	//
	// 当多个通道操作中的任意一个准备好时（可以发送或接收数据），
	// 对应的 case 就会被执行。如果没有任何通道操作准备好，
	// 则整个 select 语句会 阻塞 直到有一个通道操作变为可用。
	// 一旦选择了某个操作并执行完毕，select 语句结束，不会自动进入下一轮选择。
	select {
	case e.ch <- req: // 通过通道 ch 向服务器发送请求，若网络已销毁则返回 false
...
func MakeNetwork() *Network {
	...
	rn.endCh = make(chan reqMsg) // 这个channel就是所有ClientEnd的channel  MakeEnd()函数中绑定
