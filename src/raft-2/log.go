...
type Log struct {
	...
	snapshot           Snapshot
	hasPendingSnapshot bool // applySnapshot  true if the snapshot is not yet delivered to the application.

	// persisted log entries. log
	entries []Entry

	// TODO: rename applied with delivered. Update comments and docs as well.
	applied   uint64 //  commitIndex the highest log index of the log entry raft knows that the application has applied.
	committed uint64 // lastApplied the highest log index of the log entry raft knows that the raft cluster has committed.
