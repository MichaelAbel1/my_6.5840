...
func (rf *Raft) handleInstallSnapshotReply(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	// !! sendInstallSnapshot发出去后阻塞在网络中，再回来时，状态可能已经发生改变 args.Term == rf.term && rf.lagBehindSnapshot(m.From)保证回复和发送的rpc相符
	if rf.state == Leader && args.Term == rf.term && rf.lagBehindSnapshot(reply.From) {
		if reply.CaughtUp {
			// DPrintf("Server %v %p (Term: %v) Snapshot to %v done, args.LastIncludedIndex: %v, rf.matchIndex[peer]: %v", rf.me, rf, rf.term, peer, args.LastIncludedIndex, rf.matchIndex[peer])
