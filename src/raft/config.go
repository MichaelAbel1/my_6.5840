...
func make_config(t *testing.T, n int, unreliable bool, snapshot bool) *config {
	...
	runtime.GOMAXPROCS(4) // 设置Go程序可以同时使用的最大CPU核心数为4
	cfg := &config{}
	cfg.t = t
...
func (cfg *config) crash1(i int) {
	...
	if cfg.saved[i] != nil {
		DPrintf("config.go: crash1 Server %v : cfg.saved[i] != nil", i)
...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v { // 如果在已经提交了日志，但后面的日志又覆盖前面的，或者说相同索引但是日志不一样就报错 否则跳过
			log.Printf("%v: log %v; server %v\n", i, cfg.logs[i], cfg.logs[j]) // i 为服务器的index
			// some server has already committed a different value for this entry!
			err_msg = fmt.Sprintf("commit index=%v server=%v %v != server=%v %v",
				m.CommandIndex, i, m.Command, j, old)
		}
	}
	_, prevok := cfg.logs[i][m.CommandIndex-1] // 应用日志
	cfg.logs[i][m.CommandIndex] = v            // 所有服务器都存在日志v后才会应用
...
func (cfg *config) applier(i int, applyCh chan ApplyMsg) {
	for m := range applyCh {
		if m.CommandValid == false {
			...
			cfg.mu.Lock()                          // checkLogs 函数 检查并在服务器（i）上应用日志
			err_msg, prevok := cfg.checkLogs(i, m) // i 为服务器index m是消息 prevok判断m的命令的前一条日志是否存在  checkLogs函数检查并应用日志
			cfg.mu.Unlock()
...
func (cfg *config) applierSnap(i int, applyCh chan ApplyMsg) {
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			...
			cfg.mu.Lock()
			cfg.lastApplied[i] = m.CommandIndex // 记录已经应用日志的index
			cfg.mu.Unlock()

			if (m.CommandIndex+1)%SnapShotInterval == 0 { // 到一定index条目就生成快照
				w := new(bytes.Buffer)
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	svc := labrpc.MakeService(rf) // 创建服务
	srv := labrpc.MakeServer()    // 创建服务器 和上面不一样
	srv.AddService(svc)
...
// how many servers think a log entry is committed?  有多少个服务器日志已经到index
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		cfg.mu.Lock()
		cmd1, ok := cfg.logs[i][index] // 获取raft服务器i 日志编号为index处的命令
		cfg.mu.Unlock()
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		for si := 0; si < cfg.n; si++ {
			...
			cfg.mu.Unlock()
			if rf != nil {
				index1, _, ok := rf.Start(cmd) // 如果是leader则提交日志返回true，否则返回false
				if ok {
					index = index1
					break
				}
