...
func TestInitialElection3A(t *testing.T) {
	...
	DPrintf("Start sleeping for 2000ms !!!")
...
func TestBasicAgree3B(t *testing.T) {
	servers := 3
	cfg := make_config(t, servers, false, false) // 使得服务器间相互连接
	defer cfg.cleanup()
...
func TestPersist13C(t *testing.T) {
	...
	// DPrintf("**************************************")
	// DPrintf("cfg.one(11, servers, true) has finished")
	// DPrintf("**************************************")
	// DPrintf("config.go: crash1 Server %v : cfg.saved[i] : %v", 0, cfg.saved[0])
	// DPrintf("config.go: crash1 Server %v : cfg.saved[i] : %v", 1, cfg.saved[1])
	// DPrintf("config.go: crash1 Server %v : cfg.saved[i] : %v", 2, cfg.saved[2])
	// DPrintf("**************************************")
