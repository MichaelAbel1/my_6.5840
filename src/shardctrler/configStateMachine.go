...
func (cf *MemoryConfigStateMachine) Join(groups map[int][]string) Err {
	DPrintf("MemoryConfigStateMachine.Join: adds new groups: %v to the configuration ", groups)
	lastConfig := cf.Configs[len(cf.Configs)-1]
	// create a new configuration based on the last configuration
	newConfig := Config{
		len(cf.Configs),
		lastConfig.Shards,           // 值类型（固定大小的数组），不需要深拷贝
		deepCopy(lastConfig.Groups), // 引用类型（切片或映射），因此需要深拷贝
	}
	...
	group2Shards := Group2Shards(newConfig) // 将 shard 分配到对应group中
	for {
	...
	DPrintf("*****************************************")
	DPrintf("MemoryConfigStateMachine.Join: %v succuss!!!", groups)
	for sn, gid := range newConfig.Shards {
		DPrintf("MemoryConfigStateMachine.Join: SN: %v -> GID: %v", sn, gid)
	}
	DPrintf("MemoryConfigStateMachine.Join: len(cf.Configs):%v", len(cf.Configs))
	DPrintf("*****************************************")
	return OK
}
...
func (cf *MemoryConfigStateMachine) Leave(gids []int) Err {
	DPrintf("MemoryConfigStateMachine.Leave: remove specified groups: %v from the configuration ", gids)
	...
	for _, gid := range gids {
		...
		if _, ok := newConfig.Groups[gid]; ok {
			delete(newConfig.Groups, gid) // delete 操作对不存在的键不会引发错误，也不会对 map 产生不良影响 判断多余
		}
	...
	if len(newConfig.Groups) > 0 {
		...
		for _, shard := range orphanShards {
			...
			newShards[shard] = gid // !!!一步没必要 在后面加入也一样
	...
	cf.Configs = append(cf.Configs, newConfig)
	DPrintf("*****************************************")
	DPrintf("MemoryConfigStateMachine.Leave: %v succuss!!!", gids)
	for sn, gid := range newConfig.Shards {
		DPrintf("MemoryConfigStateMachine.Join: SN: %v -> GID: %v", sn, gid)
	}
	DPrintf("MemoryConfigStateMachine.Leave: len(cf.Configs):%v", len(cf.Configs))
	DPrintf("*****************************************")
	return OK
}
...
func (cf *MemoryConfigStateMachine) Move(shard, gid int) Err {
	DPrintf("MemoryConfigStateMachine.Move: move a specified shard: %v to a specified group: %v", shard, gid)
	lastConfig := cf.Configs[len(cf.Configs)-1]
	// create a new configuration based on the last configuration
	newConfig := Config{
		len(cf.Configs),
	...
	cf.Configs = append(cf.Configs, newConfig)
	DPrintf("*****************************************")
	DPrintf("MemoryConfigStateMachine.Move: %v succuss!!!", shard)
	for sn, gid := range newConfig.Shards {
		DPrintf("MemoryConfigStateMachine.Move: SN: %v -> GID: %v", sn, gid)
	}
	DPrintf("MemoryConfigStateMachine.Move: len(cf.Configs):%v", len(cf.Configs))
	DPrintf("*****************************************")
	return OK
}
...
func (cf *MemoryConfigStateMachine) Query(num int) (Config, Err) {
	...
	DPrintf("MemoryConfigStateMachine.Query: len(cf.Configs):%v", len(cf.Configs))
...
func GetGIDWithMinimumShards(group2Shards map[int][]int) int {
	...
	// FAIL: TestMulti/AgreeConfig 如果去掉就无法保证获得的index是一样的，因为在在Go中，map的迭代顺序是不确定的，
	// 如果出现有两个最小长度的len(group2Shards[gid]，就会出现得到的index不相同
	sort.Ints(gids)
