...
type ShardKV struct {
	...
	persister      *raft.Persister            // save the state machine to disk
}
...
func (kv *ShardKV) Execute(command Command, reply *CommandReply) {
	...
	defer DPrintf("{Node %v}{Group %v} Execute Command %v with CommandReply %v", kv.me, kv.gid, command, reply)
	kv.mu.Lock()
...
// checks if the server can serve the shard.
...
func StartServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int, gid int, ctrlers []*labrpc.ClientEnd, make_end func(string) *labrpc.ClientEnd) *ShardKV {
	...
	DPrintf("{Node %v}{Group %v} started", kv.me, kv.gid)
...
func (kv *ShardKV) gcAction() {
	...
	for gid, shardIDs := range gid2Shards {
		DPrintf("{Node %v}{Group %v} starts a GCTask to delete shards %v from group %v when config is %v", kv.me, kv.gid, shardIDs, gid, kv.currentConfig)
		wg.Add(1)
...
func (kv *ShardKV) GetShardsData(args *ShardOperationArgs, reply *ShardOperationReply) {
	...
	kv.mu.RLock()
	defer kv.mu.RUnlock()
	defer DPrintf("{Node %v}{Group %v} processes PullTaskRequest %v with PullTaskReply %v", kv.me, kv.gid, args, reply)
...
func (kv *ShardKV) DeleteShardsData(args *ShardOperationArgs, reply *ShardOperationReply) {
	...
	defer DPrintf("{Node %v}{Group %v} processes GCTaskRequest %v with GCTaskReply %v", kv.me, kv.gid, args, reply)
	kv.mu.RLock()
	// Check if the current configuration is greater than the requested one.
	if kv.currentConfig.Num > args.ConfigNum {
		DPrintf("{Node %v}{Group %v} encounters duplicated shards deletion request %v when currentConfig is %v", kv.me, kv.gid, args, kv.currentConfig)
		reply.Err = OK
		kv.mu.RUnlock()
		return
	}
...
func (kv *ShardKV) configurationAction() {
	...
	for _, shard := range kv.stateMachine {
		if shard.Status != Serving {
			...
			DPrintf("{Node %v}{Group %v} will not try to fetch latest configuration because shards status are %v when currentConfig is %v", kv.me, kv.gid, kv.stateMachine, kv.currentConfig)
			break
		}
	}
	...
	for !kv.killed() {
		...
		if message.CommandValid {
			...
			if message.CommandIndex <= kv.lastApplied {
				DPrintf("{Node %v}{Group %v} discards outdated message %v because a newer snapshot which lastApplied is %v has been applied", kv.me, kv.gid, message, kv.lastApplied)
				kv.mu.Unlock()
				continue
			}
			// update the last applied index
			kv.lastApplied = message.CommandIndex // raft保证了会按递增顺序提交
			...
			// message_term, err := kv.rf.GetTermWithoutLock(message.CommandIndex)
...
func (kv *ShardKV) applyOperation(operation *CommandArgs) *CommandReply {
	...
	if !kv.canServe(shardID) {
		...
		if operation.Op != Get && kv.isDuplicateRequest(operation.ClientId, operation.CommandId) {
			DPrintf("{Node %v}{Group %v} does not apply duplicated commandId %v to stateMachine because maxAppliedCommandId is %v for clientId %v", kv.me, kv.gid, operation.CommandId, kv.lastOperations[operation.ClientId].MaxAppliedCommandId, operation.ClientId)
...
func (kv *ShardKV) applyConfiguration(nextConfig *shardctrler.Config) *CommandReply {
	...
	if nextConfig.Num == kv.currentConfig.Num+1 {
		...
		DPrintf("{Node %v}{Group %v} discards outdated configuration %v when currentConfig is %v", kv.me, kv.gid, nextConfig, kv.currentConfig)
...
func (kv *ShardKV) applyInsertShards(shardsInfo *ShardOperationReply) *CommandReply {
	...
	if shardsInfo.ConfigNum == kv.currentConfig.Num {
		...
		DPrintf("{Node %v}{Group %v} discards outdated shards insertion %v when currentConfig is %v", kv.me, kv.gid, shardsInfo, kv.currentConfig)
		reply.Err = ErrOutDated
	}
	return reply
}
...
func (kv *ShardKV) applyDeleteShards(shardsInfo *ShardOperationArgs) *CommandReply {
	...
	if shardsInfo.ConfigNum == kv.currentConfig.Num {
		DPrintf("{Node %v}{Group %v}'s shards status are %v before accepting shards deletion %v when currentConfig is %v", kv.me, kv.gid, kv.stateMachine, shardsInfo, kv.currentConfig)
		...
		DPrintf("{Node %v}{Group %v}'s shards status are %v after accepting shards deletion %v when currentConfig is %v", kv.me, kv.gid, kv.stateMachine, shardsInfo, kv.currentConfig)
		return &CommandReply{Err: OK}
	}
	DPrintf("{Node %v}{Group %v} discards outdated shards deletion %v when currentConfig is %v", kv.me, kv.gid, shardsInfo, kv.currentConfig)
	return &CommandReply{Err: OK}
}
